...
void data_callback(ma_device* device, void* output, void const*, ma_uint32 frame_count) {
...
bool AudioManager::push_pcm_data(sptr<pcm_data> const& pcm) const {
    if (!audio_system.is_initialized.load() || !pcm) {
        return false;
    }

    std::lock_guard<std::mutex> lock(audio_system.pending_mutex);
    audio_system.pending_handles.emplace_back(audio_hnd(pcm, 0));
