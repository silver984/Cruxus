...
sptr<std::vector<float>> ResourceSys::load_pcm_data(std::string_view path) {
    ...
    pcm->resize(static_cast<size_t>(total_frames * CRX_AUDIO_CHANNELS));
    ...
    while (total_read < total_frames) {
        ...
        ma_result r = ma_decoder_read_pcm_frames(
            &decoder,
            pcm->data() + total_read * CRX_AUDIO_CHANNELS,
    ...
    pcm->resize(static_cast<size_t>(total_read * CRX_AUDIO_CHANNELS));
