...
namespace slv::raylib
{
	...
	void draw_texture(const slv::texture& texture, const slv::rect<float>& source, const slv::rect<float>& dest, const slv::vec_2<float>& anchor, float rotation, float alpha, const slv::rgb& color = slv::white);
